...
class _NegotiationDetailScreenState extends ConsumerState<NegotiationDetailScreen> {
  ...
  Future<void> _fetchDetail() async {
    ...
    try {
      ...
      if (response.data['success'] == true) {
        setState(() { _negotiation = response.data['data']; _isLoading = false; });
      }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Offer accepted!', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600)),
            backgroundColor: greenAccent, behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
        );
        context.pop(true);
      }
    } on DioException catch (e) {
      _showError(e.response?.data?['message']?.toString() ?? 'Failed to accept');
    } finally {
  ...
  Future<void> _rejectNegotiation() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Cancel Negotiation?', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700)),
        content: Text('This action cannot be undone.', style: GoogleFonts.plusJakartaSans()),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('No')),
          TextButton(onPressed: () => Navigator.pop(ctx, true),
            child: Text('Yes, Cancel', style: GoogleFonts.plusJakartaSans(color: redAccent))),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Negotiation cancelled', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600)),
            backgroundColor: redAccent, behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
        );
        context.pop(true);
      }
  ...
  Future<void> _submitCounterOffer() async {
    ...
    try {
      final api = ref.read(apiClientProvider);
      await api.post('/negotiations/${widget.negotiationId}/counter', data: {
        'pricePerUnit': price,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Counter offer sent!', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600)),
      ...
      _showError(e.response?.data?['message']?.toString() ?? 'Failed to send counter');
    } finally {
      if (mounted) setState(() => _isActioning = false);
    }
  }
  ...
  Future<void> _proceedToOrder() async {
    ...
    try {
      ...
      debugPrint('Address dialog returned: $address');
      ...
      try {
        ...
        _showError(e.response?.data?['message']?.toString() ?? 'Failed to create order');
      } finally {
        if (mounted) setState(() => _isActioning = false);
      }
    } catch (e) {
      ...
      builder: (ctx) => Container(
        ...
        child: Padding(
          ...
          child: Form(key: formKey, child: SingleChildScrollView(child: Column(
            ...
            const SizedBox(height: 12),
            _addrField('Pincode', pinCtrl, keyboard: TextInputType.number),
            const SizedBox(height: 20),
            SizedBox(width: double.infinity, height: 52,
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: greenAccent, foregroundColor: Colors.white,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
  ...
  Widget _addrField(String label, TextEditingController ctrl, {TextInputType? keyboard}) {
    return TextFormField(controller: ctrl, keyboardType: keyboard,
      ...
      decoration: InputDecoration(
        ...
        filled: true, fillColor: backgroundWhite,
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: borderLight)),
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: borderLight)),
        ...
        builder: (ctx, setSheetState) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Your Counter Offer', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w700, color: textPrimary)),
              const SizedBox(height: 4),
              Text('For $quantity units', style: GoogleFonts.plusJakartaSans(fontSize: 14, color: slateBlue)),
              const SizedBox(height: 20),
              Text('Price per unit (₹)', style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.w600, color: textPrimary)),
              ...
              const SizedBox(height: 16),
              Text('Message (optional)', style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.w600, color: textPrimary)),
              const SizedBox(height: 8),
              TextField(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isActioning
                      ...
                      : Text('Send Counter Offer', style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
                ),
              ),
            ],
          ),
          ...
          child: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                      ...
                      if (imageUrl.isNotEmpty)
                        SizedBox(
                          ...
                          child: CachedNetworkImage(imageUrl: imageUrl, fit: BoxFit.cover,
                            errorWidget: (_, __, ___) => Container(color: backgroundWhite)),
                        ),
                      Expanded(
                        child: Padding(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(productName, style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: textPrimary)),
                              const SizedBox(height: 4),
                              Text('Retail: ₹${NumberFormat('#,##,###').format(originalPrice)}',
                                style: GoogleFonts.plusJakartaSans(fontSize: 13, color: slateBlue)),
                              Text('Qty: $quantity units', style: GoogleFonts.plusJakartaSans(fontSize: 13, color: slateBlue)),
                            ],
                          ),
                        ),
                      ),
                ...
                // Current Status Card
                _buildStatusCard(status, currentPrice, currentTotal, currentOfferBy, quantity),
                const SizedBox(height: 20),

                // History Timeline
                Text('Negotiation History', style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.w700, color: textPrimary)),
                const SizedBox(height: 12),
  ...
  Widget _buildStatusCard(String status, dynamic currentPrice, dynamic currentTotal, String currentOfferBy, dynamic quantity) {
    Color statusColor;
    String statusLabel;
    IconData statusIcon;
    ...
    switch (status) {
      case 'pending':
        statusColor = textMuted; statusLabel = 'Pending Review'; statusIcon = Icons.hourglass_empty_rounded;
        break;
      case 'countered':
        statusColor = amberAccent; statusLabel = currentOfferBy == 'admin' ? 'Admin Counter Offer' : 'Your Counter Offer';
        statusIcon = Icons.swap_horiz_rounded;
        break;
      case 'accepted':
        statusColor = greenAccent; statusLabel = 'Accepted'; statusIcon = Icons.check_circle_rounded;
        break;
      case 'rejected':
        statusColor = redAccent; statusLabel = 'Rejected'; statusIcon = Icons.cancel_rounded;
        break;
      default:
        statusColor = textMuted; statusLabel = status.toUpperCase(); statusIcon = Icons.info_outline;
    }
        ...
        children: [
          Row(
            children: [
              ...
              Text(statusLabel, style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: statusColor)),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            ...
            children: [
              Text('Current Price/unit', style: GoogleFonts.plusJakartaSans(fontSize: 13, color: slateBlue)),
              Text('₹${NumberFormat('#,##,###').format(currentPrice)}',
                style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.w800, color: statusColor)),
            ],
          ),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Total ($quantity units)', style: GoogleFonts.plusJakartaSans(fontSize: 13, color: slateBlue)),
              Text('₹${NumberFormat('#,##,###').format(currentTotal)}',
  ...
  Widget _buildHistoryItem(Map<String, dynamic> entry) {
    ...
    if (timestamp.isNotEmpty) {
      try {
        formattedTime = DateFormat('MMM d, h:mm a').format(DateTime.parse(timestamp));
      } catch (_) {}
    }
    ...
    switch (action) {
      case 'requested':
        dotColor = primaryBlue; dotIcon = Icons.send_rounded; actionLabel = 'Negotiation Requested';
        break;
      case 'countered':
        dotColor = amberAccent; dotIcon = Icons.swap_horiz_rounded;
        actionLabel = by == 'admin' ? 'Admin Counter Offer' : 'Your Counter Offer';
        break;
      case 'accepted':
        dotColor = greenAccent; dotIcon = Icons.check_circle_rounded;
        actionLabel = by == 'admin' ? 'Accepted by Admin' : 'You Accepted';
        break;
      case 'rejected':
        dotColor = redAccent; dotIcon = Icons.cancel_rounded;
        actionLabel = by == 'admin' ? 'Rejected by Admin' : 'You Cancelled';
        break;
      default:
        dotColor = textMuted; dotIcon = Icons.circle; actionLabel = action;
    }
          ...
          children: [
            ...
            Expanded(
              child: Container(
                ...
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                  color: surfaceWhite, borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: borderLight),
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(actionLabel, style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.w700, color: dotColor)),
                        if (formattedTime.isNotEmpty)
                          Text(formattedTime, style: GoogleFonts.plusJakartaSans(fontSize: 11, color: textMuted)),
                      ],
                    ),
                    if (price != null) ...[
                      const SizedBox(height: 6),
                      Row(
                        children: [
                          Text('₹${NumberFormat('#,##,###').format(price)}/unit',
                    ...
                    if (message.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      Text('"$message"', style: GoogleFonts.plusJakartaSans(fontSize: 13, fontStyle: FontStyle.italic, color: slateBlue)),
                    ],
                  ],
                ),
              ),
  ...
  Widget _buildBottomActions(String status, String currentOfferBy, bool canPay) {
    return Container(
  ...
  Widget _buildActionRow(String status, String currentOfferBy, bool canPay) {
    if (status == 'accepted' && canPay) {
      return SizedBox(
        ...
        child: ElevatedButton.icon(
          ...
          label: Text('Proceed to Order', style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700)),
          style: ElevatedButton.styleFrom(
            backgroundColor: greenAccent, foregroundColor: Colors.white,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ),
          ...
          Expanded(
            child: OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(
                foregroundColor: redAccent, side: const BorderSide(color: redAccent),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                padding: const EdgeInsets.symmetric(vertical: 14),
              ),
              child: Text('Decline', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700)),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            child: OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(
                foregroundColor: amberAccent, side: const BorderSide(color: amberAccent),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                padding: const EdgeInsets.symmetric(vertical: 14),
              ),
              child: Text('Counter', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700)),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            child: ElevatedButton(
              ...
              child: Text('Accept', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700)),
            ),
          ),
        ],
      );
    ...
    if (status == 'pending' || (status == 'countered' && currentOfferBy == 'wholesaler')) {
      return Row(
        children: [
          Expanded(
            child: OutlinedButton(
              ...
              child: Text('Cancel Negotiation', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700)),
            ),
          ),
        ],
      );
    ...
    if (status == 'accepted') {
      return Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: greenAccent.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.check_circle_rounded, color: greenAccent, size: 18),
            const SizedBox(width: 8),
            Text('Negotiation Completed', style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w700, color: greenAccent)),
          ],
        ),
      );
    }
